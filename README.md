...
- 📅 **热力图月历** — 折叠式热力图，直观看见这个月的情绪密度
- 📖 **翻本子动画** — 画板和精选页均实现 3D 翻页效果，装订线 + 叠层纸 + 卷页阴影，模拟手工线圈本
- 🔐 **OTP 验证码登录** — 邮箱接收验证码，当前浏览器直接输入，无需跳转。安全合并 / 本地覆盖云端 / 云端覆盖本地三种同步模式
- 📤 **投稿与精选审核** — 匿名投稿，管理员审核（入选/驳回），入选作品展示在精选挂历。本小时限投 10 次，展开投稿区可见剩余次数
- 🖼️ **精选挂历** — 每页展示 4 个表情，大小错落 + 和纸胶带粘贴 + 四种排版轮换，纸纤维纹理背景
- 📲 **分享图生成** — 保存后一键生成含表情 + 文案 + 二维码的分享图，支持系统分享或保存本地
- 📱 **PWA 就绪** — 可安装到桌面，完全离线使用，新版本自动提示更新
